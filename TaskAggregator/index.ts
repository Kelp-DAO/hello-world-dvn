  ...
  return new Promise<string>(async (resolve, reject) => {
    ...
    WalletHelper.registerDemoDVN()
      .then(({ dvnCoordinator, dvnOwner }) => {
        ...
        //
        const dvnCoordinatorContract =
          WalletHelper.getDemoDVNCoordinatorContract(
            WalletHelper.getDVNOwnerWallet(),
          );

        // listen for OperatorRegistrationRequestAccepted event
        dvnCoordinatorContract.on(
          "OperatorRegistrationRequestAccepted",
          (operatorId) => {
            logger.info(
              `Operator #${operatorId.toString()} is now registered to the DVN`,
            );
...
const printDVNInfo = async (
  dvnCoordinatorContract: Contract,
): Promise<void> => {
  logger.info("DVN Info:");
  logger.info(
    `  Operators: ${(await dvnCoordinatorContract.getOperatorCount()).toString()}`,
  );
};
...
const startTaskAggregatorServer = async (): Promise<void> => {
  return new Promise<void>(async (resolve, reject) => {
    ...
    const server = app.listen(port, () => {
      logger.info(
        `Task Aggregator server up and running on port http://localhost:${port}`,
      );
    });
 ...
 */
const main = async () => {
  ProcessHelper.waitUntilAnvilIsAvailable().then(async () => {
    // register DVN
    await registerDVN();
